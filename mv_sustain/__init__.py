...
Built on top of pySuStaIn (Young et al. 2018, Nat Commun;
https://github.com/ucl-pond/pySuStaIn), this package adds a joint
patient-level likelihood so that repeated visits from the same patient
reinforce a single subtype/stage estimate instead of being scored as
independent, unrelated observations.

Two model families are provided for each supported likelihood
(z-score and ordinal):
...
`use_longitudinal_likelihood` flag, so most users should not need to
instantiate the model classes directly.
"""
