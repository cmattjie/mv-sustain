...
class AbstractSustain(ABC):
    ...
    def __init__(self,
        ...
        # Accept either the local AbstractSustainData or the pySuStaIn base class.
        try:
            from pySuStaIn.AbstractSustain import AbstractSustainData as _PyAbstractSustainData  # type: ignore
        except Exception:
            _PyAbstractSustainData = None

        if _PyAbstractSustainData is None:
            assert isinstance(sustainData, AbstractSustainData)
        else:
            assert isinstance(sustainData, (AbstractSustainData, _PyAbstractSustainData))
        ...
        # parallel_startpoints is deprecated — always use serial pool.
        self.use_parallel_startpoints   = False
        self.requested_parallel_startpoints = bool(use_parallel_startpoints)
        self.used_parallel_startpoints  = False
        self.parallel_startpoints_fallback_reason = "deprecated"
    ...
    def run_sustain_algorithm(self, plot=False, plot_format="png", **kwargs):
        ...
        for s in range(self.N_S_max):
            ...
            # Integrate assignments over the full fitted posterior.
            N_samples                       = int(samples_sequence.shape[2])
            ml_subtype,             \
            prob_ml_subtype,        \
            ml_stage,               \
            prob_ml_stage,          \
    ...
    def _optimise_mcmc_settings(self, sustainData, seq_init, f_init):
        ...
        n_iterations_MCMC_optimisation      = int(self.N_iterations_MCMC)
